        ...
        for model in GEMINI_MODEL_HIERARCHY:
            for key in GEMINI_KEYS:
                try:
                    ...
                    pass
        raise RuntimeError("No LLM instance could be created.")
...
# -----------------------------
# Runner: orchestrates LLM call -> pre-scrape inject -> execute
# -----------------------------
def run_agent_safely(llm_input: str) -> Dict:
    """
    1. Call LLM directly with the input prompt
    ...
    try:
        ...
        if not isinstance(parsed, dict) or "code" not in parsed or "questions" not in parsed:
            return {"error": f"Invalid LLM response format: {parsed}"}
...
def run_agent_safely_unified(llm_input: str, pickle_path: str = None) -> Dict:
    """
    Calls the LLM directly and executes generated code.
    - Retries up to 3 times if LLM returns no usable output.
